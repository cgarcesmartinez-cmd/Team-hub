  ...
  useEffect(() => {
    function load() {
      try {
        ...
        else { setMembers(INITIAL_MEMBERS); localStorage.setItem("hub-members", JSON.stringify(INITIAL_MEMBERS)); }
      } catch { setMembers(INITIAL_MEMBERS); localStorage.setItem("hub-members", JSON.stringify(INITIAL_MEMBERS)); }
      try {
        const t = localStorage.getItem("hub-tasks");
        if (t) setTasks(JSON.parse(t));
        else { setTasks(INITIAL_TASKS); localStorage.setItem("hub-tasks", JSON.stringify(INITIAL_TASKS)); }
      } catch { setTasks(INITIAL_TASKS); localStorage.setItem("hub-tasks", JSON.stringify(INITIAL_TASKS)); }
      try {
